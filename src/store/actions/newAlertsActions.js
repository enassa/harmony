...
export const getNewAlerts= (limit = undefined, initial=undefined) => {
    ...
    if(limit !== undefined){
            ...
            url = `${URL_END_POINTS.GET_NEW_ALERTS}?_start=${lowerLimit}&_limit=${upperLimit}`
            lowerLimit = lowerLimit + limit 
            return async (dispatch, getState) => {
                axios.get(url).then((response) => {
                    dispatch({type: ACTION_TYPES.GET_NEW_ALERTS, payload:response.data})
                }).catch((error) => {})
            }
    }
}
