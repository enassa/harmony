...
import CustomRedirect from '../../reusable/CustomRedirect';
import { PROJECT_OBJECT, STATUS_UPDATE } from '../../contants/Contentobject';
import { clearLocalStorage, saveObjectInLocalStorage, saveObjectInSession, removeItemFromLocalStorage, searchContains } from '../../contants/generalFunctions';
import { changeLoadingState } from './AppStateActions';
// GET PROJECT ACTION
let lastDataId = 0;
let innitialLoad ='';
// export const getProjects = (limit = undefined,initial=undefined) => {
...
//         }
//         let url =  URL_END_POINTS.GET_PROJECTS;
//         if(limit !== undefined){
//             url = `${URL_END_POINTS.GET_PROJECTS}?_start=${lastDataId}&_limit=${lastDataId+limit}`
//             lastDataId = lastDataId + limit
//         }
//         return async (dispatch, getState) => {
//             axios.get(url).then((response) => {
//                 dispatch({type: ACTION_TYPES.GET_PROJECTS, payload:response.data})
//             }).catch((error) => {})
//         }
// }
// export const openProject = (project,changePage=()=>{}) => {
//     <CustomRedirect link={`${ALL_URLS.memberListProjects}/${project.id}`}/>
//     return async (dispatch, getState) => {
